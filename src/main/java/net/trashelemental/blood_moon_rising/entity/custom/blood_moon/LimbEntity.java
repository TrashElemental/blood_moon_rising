...
public class LimbEntity extends TamableWoundMob implements GeoEntity {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(6, new FollowOwnerGoal(this, 1, 10f, 2f) {
            @Override
            public boolean canUse() {
                if (!(behavior.equals(Behavior.FOLLOW))) return false;
                return super.canUse();
    ...
    boolean isLimbAttackTarget(LivingEntity entity) {
        return (!invalidTarget(entity) && (entity.hasEffect(ModMobEffects.SCORN) || entity instanceof ParasiteEntity)
                && behavior.equals(Behavior.WANDER));
    }
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (!this.isTame() && itemstack.is(ModItems.PARASITE_EGGS.get())) {
            if (!player.hasEffect(ModMobEffects.KINSHIP)) return InteractionResult.PASS;
        }
    ...
    protected void defineSynchedData(SynchedEntityData.Builder builder) {
        ...
        builder.define(HAS_ORGANELLE, false);
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        ItemStack carried = this.getCarriedItem();
        if (!carried.isEmpty()) {
            Tag savedTag = carried.save(this.level().registryAccess());
            tag.put("CarriedItem", savedTag);
        }
    }
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        if (tag.contains("CarriedItem")) {
            ...
            this.setCarriedItem(ItemStack.parseOptional(this.level().registryAccess(), itemTag));
        } else {
            this.setCarriedItem(ItemStack.EMPTY);
        }
    }
    ...
    public ResourceLocation getTexture() {
        if (this.entityData.get(HAS_ORGANELLE)) {
            return BloodMoonRising.prefix("textures/entity/limb_with_organelle.png");
        }
