...
public class BileProjectileEntity extends AbstractArrow implements GeoEntity {
    ...
    public static boolean hasTaintedHeart(Player player) {
        return player.getCapability(ModCapabilities.HEART_DATA)
                .map(data -> data.hasHeart(ModItems.TAINTED_HEART.get()))
                .orElse(false);
    }
    ...
    protected void onHitEntity(EntityHitResult result) {
        ...
        if (!this.level().isClientSide && entity instanceof LivingEntity target && shouldHurt(entity)) {
            if (this.random.nextFloat() < corrosionChance && (!target.hasEffect(ModMobEffects.CORROSION.get()))) {
                target.addEffect(new MobEffectInstance(ModMobEffects.CORROSION.get(), 200));
                if (entity instanceof Player player) {
